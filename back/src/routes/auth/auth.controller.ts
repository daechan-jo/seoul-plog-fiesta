...
import { mailSendApp } from '../../utils/mailSend';
...
export class AuthController {
  ...
  async login(req: any, res: Response, next: NextFunction) {
    ...
    try {
      const { id } = req.user;
      // todo N+1 문제 해결
      const groups = await this.authService.getGroupsByUserId(id);
      const friendships = await this.authService.getFriendIdsByUserId(id);
      const user = {
        id,
        token: req.token,
  ...
  async sendEmailWithTokenUrl(req: Request, res: Response, next: NextFunction) {
    /**
     * #swagger.tags = ['Auth']
    ...
    try {
      ...
      // 이메일 발송
      await mailSendApp(emailOptions);
      // 사용자의 토큰 업데이트
      const user = await this.authService.updatePasswordTokenByEmail(
        email,
        token,
      );
      return res.status(200).json(user);
  ...
  async checkEmail(req: Request | any, res: Response, next: NextFunction) {
    /**
     * #swagger.tags = ['Auth']
    ...
    try {
      ...
      const user = await this.authService.getUserByPasswordToken(token);

      // 유저에 PasswordValid, 토큰 유효 기간 기록
      const updatedUser = await this.authService.updatePasswordValidByEmail(
        user.email,
      );
  ...
  async unLoginUpdatePassword(req: Request, res: Response, next: NextFunction) {
    /**
     * #swagger.tags = ['Auth']
    ...
    try {
      ...
      const updatedUser = await this.authService.unLoginUpdatePassword(
        email,
        password,
      );
  ...
  async changeInformation(req: Request, res: Response, next: NextFunction) {
    /**
     * #swagger.tags = ['Auth']
    ...
    try {
      ...
      const changedUser = await this.authService.changeInformation(user);
  ...
  async loginChangePassword(req: Request, res: Response, next: NextFunction) {
    /**
     * #swagger.tags = ['Auth']
    ...
    try {
      ...
      const updatedUser = await this.authService.loginChangePassword(
        req.user.id,
        password,
        newPassword,
      );
